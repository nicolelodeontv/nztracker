...
export default function WarOps({ rows, server, updated, status, onOpenClan }) {
  ...
  useEffect(() => {
    ...
    rows.forEach((row) => {
      ...
      if (Number.isFinite(oldRep) && rep !== oldRep) {
        const delta = rep - oldRep;
        setEvents(pushEvent({ clan: row.clan, type: delta > 0 ? 'gain' : 'loss', value: delta, at: Date.now() }));
      }
      const rank = Number(row.rank || 0);
      const oldRank = Number(rankStored[row.clan]);
      if (Number.isFinite(oldRank) && oldRank > 0 && rank !== oldRank) {
        setEvents(pushEvent({ clan: row.clan, type: 'rank', value: `#${oldRank} → #${rank}`, at: Date.now() }));
      }
  ...
  const targetBleeding = target?.status?.state === 'bleeding';
  const result = target && ownRep !== '' ? getVictoryResult(own, targetRep, targetBleeding) : null;
  ...
  return <section className="war-ops">
    <div className="war-ops-bar">
      {[['all','ALL'],['watch',`★ WATCHLIST ${watchlist.length ? `· ${watchlist.length}` : ''}`],['war','⚔ CLAN WAR'],['bleed',`🔴 BLEEDING ${bleeding.length ? `· ${bleeding.length}` : ''}`]].map(([key,label]) => <button key={key} className={`war-ops-tab ${key === 'bleed' ? 'bleed' : ''} ${view === key ? 'active' : ''}`} onClick={() => setView(key)}>{label}</button>)}
    ...
    <div className="war-ops-grid">
      <div className="war-ops-panel">
        ...
        <div className="war-calculator">
          ...
          {result?.won && <div className="member-reason">Target is confirmed Bleeding. Victory reward follows the current Reputation-difference tier.</div>}
        </div>
      </div>
      ...
      <div className="war-ops-panel">
        ...
        <div className="war-status-list">
          {watched.slice(0, 8).map((item) => <div className="war-status-item" key={item.clan}><button className="war-feed-click" onClick={() => open(item.clan)}><span className="war-status-name">{item.clan}</span><span className="war-status-meta">{statusLabel(item.status.state)} · {item.status.state === 'bleeding' ? `${item.status.bleedingMembers}/${item.status.memberCount} below threshold` : item.status.reason || 'Healthy'}</span></button><button className="war-badge unknown" onClick={() => toggleWatch(item.clan)}>★ REMOVE</button></div>)}
          {!watched.length && <div className="empty">Use the star controls in the clan ranking to monitor clans here.</div>}
        </div>
      </div>
    </div>

    <div className="war-ops-panel">
      <div className="war-ops-head"><h3>🩸 STAMINA BREAKDOWN</h3><small>{focus ? focusClan : 'SELECT A CLAN'}</small></div>
      {focus?.staminaAvailable ? <div className="war-ops-list">{(focus.members || []).slice().sort((a,b) => Number(a.current)-Number(b.current)).map((member) => <div className="war-status-item" key={member.name}><span><span className="war-status-name">{member.name}</span><span className="war-status-meta">Max {fmt(member.max)} · 70% threshold {fmt(member.bleedingThreshold)} · 50% floor {fmt(member.drainFloor)}</span></span><span className={`member-stamina ${member.bleeding ? 'bleed' : 'healthy'}`}>{fmt(member.current)} / {fmt(member.max)}</span></div>)}</div> : <div className="member-reason">{focus?.reason || 'Select a clan from Bleeding Now, Watchlist, or the status filters. If the source does not expose Stamina, the status remains UNKNOWN.'}</div>}
    </div>
