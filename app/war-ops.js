...
export default function WarOps({ rows, server, updated, status, onOpenClan }) {
  ...
  const sendDiscord = async (payload) => {
    ...
    try {
      await fetch('/api/discord/attack-summary', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    } catch {}
  };
  ...
  return <section className="war-ops">
    ...
    <div className="war-ops-grid">
      <div className="war-ops-panel"><div className="war-ops-head"><h3>⚔ WAR CALCULATOR</h3><small>VICTORY ONLY</small></div><div className="war-calculator"><div className="war-calc-grid"><div className="war-calc-field"><label>YOUR CLAN REP</label><input inputMode="numeric" value={ownRep} onChange={(e)=>setOwnRep(e.target.value.replace(/[^0-9]/g,''))} placeholder="267419"/></div><div className="war-calc-field"><label>TARGET CLAN</label><select value={targetClan} onChange={(e)=>setTargetClan(e.target.value)}><option value="">Select target…</option>{rows.map((row)=><option key={row.clan} value={row.clan}>{row.clan} · {fmt(row.reputation)}</option>)}</select></div></div><div className="war-calc-field"><label>PARTY MEMBERS</label><div className="war-party">{[0,1,2].map((size)=><button key={size} className={partySize===size?'active':''} onClick={()=>setPartySize(size)}>{size===0?'SOLO':`+${size} PARTY`}</button>)}</div></div><div className="war-result"><div className="war-result-item"><small>REP DIFFERENCE</small><b>{result?`${result.difference>=0?'+':'−'}${fmt(Math.abs(result.difference))}`:'—'}</b></div><div className="war-result-item"><small>RESULT</small><b>{result?(result.won?'✅ WIN':'❌ LOSS'):targetState==='unknown'&&targetClan?'⚪ UNKNOWN':'—'}</b></div><div className="war-result-item"><small>REWARD</small><b>{result?`${result.reputation} REP`:targetState==='unknown'&&targetClan?'UNKNOWN':'—'}</b></div><div className="war-result-item"><small>DRAIN</small><b>{drainTargets} × 10 STA</b></div></div>{targetState==='unknown'&&targetClan&&<div className="member-reason">The source does not expose enough Stamina data to verify Bleeding, so the calculator will not invent a win/loss result.</div>}{result&&!targetBleeding&&<div className="member-reason">Target is not Bleeding. Quick Battle = loss and 0 Reputation.</div>}{result?.won&&<div className="member-reason">Target is Bleeding. Victory reward follows the current Reputation-difference tier.</div>}</div></div>
