...
export default function Home() {
  ...
  const load=useCallback(async()=>{try{setStatus('loading');const res=await fetch('/api/clan-ranking',{cache:'no-store'});if(!res.ok)throw new Error();const data=await res.json();const now=new Date().toISOString();setRows(current=>{const snap={};current.forEach(r=>snap[r.clan]=r.rank);if(Object.keys(snap).length)setPrevious(snap);if(data.rows?.length)saveHistory({capturedAt:now,season:data.season,rows:data.rows});return data.rows||[];});setSeason(data.season||'Season 2');setUpdatedAt(new Date(now));setStatus('live');}catch{setStatus('error');}},[saveHistory]);
