...
export default function Home() {
  ...
  const [autoRefresh,setAutoRefresh]=useState(true),[countdown,setCountdown]=useState(10),[query,setQuery]=useState(''),[sort,setSort]=useState('rank');
  ...
  const load=useCallback(async()=>{try{setStatus('loading');const res=await fetch('/api/clan-ranking',{cache:'no-store'});if(!res.ok)throw new Error();const data=await res.json();const now=new Date().toISOString();setRows(current=>{const snap={};current.forEach(r=>snap[r.clan]=r.rank);if(Object.keys(snap).length)setPrevious(snap);if(data.rows?.length)saveHistory({capturedAt:now,season:data.season,rows:data.rows});return data.rows||[];});setSeason(data.season||'Season 2');setUpdatedAt(new Date(now));setStatus('live');setCountdown(10);}catch{setStatus('error');}},[saveHistory]);
  ...
  useEffect(()=>{if(!autoRefresh)return;const t=setInterval(()=>setCountdown(v=>v<=1?10:v-1),1000);return()=>clearInterval(t);},[autoRefresh]);
