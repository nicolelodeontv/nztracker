...
export default function Home() {
  ...
  useEffect(()=>{if(!autoRefresh||!selectedClan)return;const t=setInterval(()=>{const clan=rows.find(r=>r.clan===selectedClan.clan);if(clan)loadMembers(clan);},REFRESH_MS);return()=>clearInterval(t);},[autoRefresh,selectedClan,rows,loadMembers]);
  ...
  return <main className="shell">
    ...
    {selectedClan&&<div className="memberModal" role="dialog" aria-modal="true" aria-label={`${selectedClan.clan} members`} onMouseDown={e=>e.target===e.currentTarget&&setSelectedClan(null)}><div className="memberModalBox"><div className="memberModalHeader"><div><div className="panelLabel">LIVE MEMBERS · CLAN #{selectedClan.rank}</div><h2>{selectedClan.clan}</h2><div className="memberModalMeta">Master: {selectedClan.master||'—'} · {selectedClan.memberCurrent}/{selectedClan.memberMax} members</div></div><div className="memberModalActions"><span className="memberStatus">{memberStatus==='live'?`● LIVE · ${formatTime(memberUpdatedAt)}`:memberStatus==='loading'?'Loading…':memberStatus==='error'?'Fetch error':'Unavailable'}</span><button className="modalClose" onClick={()=>setSelectedClan(null)} aria-label="Close members">×</button></div></div><div className="memberModalBody">{memberStatus==='loading'&&<div className="memberEmpty">Fetching live member names, levels and reputation…</div>}{memberStatus==='error'&&<div className="memberEmpty">Unable to fetch live members right now.</div>}{memberStatus==='unavailable'&&<div className="memberEmpty">No clan ID was returned by the source.</div>}{memberStatus==='live'&&!members.length&&<div className="memberEmpty">This clan currently has no members returned by the source.</div>}{members.length>0&&<div className="memberTable"><div className="memberHead"><span>#</span><span>MEMBER</span><span>LEVEL</span><span>REPUTATION</span><span>Δ REP</span></div>{members.map((member,index)=>{const delta=typeof memberPrevious[member.name]==='number'?member.reputation-memberPrevious[member.name]:0;return <div className="memberRow" key={`${member.name}-${index}`}><span className="memberRank">{index+1}</span><strong>{member.name}</strong><span>{member.level||'—'}</span><span>{formatNumber(member.reputation)}</span><span className={delta>0?'movement up':delta<0?'movement down':'movement same'}>{delta>0?`+${formatNumber(delta)}`:delta<0?formatNumber(delta):'—'}</span></div>})}</div>}</div><div className="memberModalFooter">Auto-refreshes every 30 seconds while this clan is open · Press Esc to close</div></div></div>}
