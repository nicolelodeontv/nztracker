...
export default function Home() {
  ...
  return <main className="shell">
    ...
    <section className="tableCard"><div className="tableHead"><span>RANK</span><span>CLAN</span><span>MASTER</span><span>MEMBERS</span><span>REPUTATION</span><span>REP/MIN</span><span>★</span></div><div className="rows">{filtered.map(row=>{const repGain=typeof previousRep[row.clan]==='number'&&row.reputation>previousRep[row.clan];const repPerMin=observedRepPerMinute(row,history);return <div key={`${row.clan}-${row.rank}`} className={`tableRow ${repGain?'repGainRow':''}`} onClick={()=>selectClan(row)} role="button" tabIndex={0} onKeyDown={e=>e.key==='Enter'&&selectClan(row)}><div className={`rank r${row.rank}`}>{row.rank<=3?['♛','◆','◆'][row.rank-1]:`#${row.rank}`}</div><div className="clan"><strong>{row.clan}</strong><div className="bar"><i style={{width:`${row.memberMax?Math.min(100,row.memberCurrent/row.memberMax*100):0}%`}}/></div></div><div className="master">{row.master||'—'}</div><div className="members">{row.memberCurrent}/{row.memberMax}</div><div className={`rep ${repGain?'repGain':''}`}>{formatNumber(row.reputation)}{repGain&&<span className="repPop">+ REP</span>}</div><div className="repRate">{repPerMin>0?`+${formatNumber(repPerMin)}`:'—'}</div><button className={favorites.includes(row.clan)?'star active':'star'} onClick={e=>{e.stopPropagation();toggleFavorite(row.clan)}}>{favorites.includes(row.clan)?'★':'☆'}</button></div>})}{!filtered.length&&<div className="empty">No clans match your search.</div>}</div></section>
