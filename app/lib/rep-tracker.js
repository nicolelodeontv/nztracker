...
export async function syncTracker({force=false,admin='system'}={}){const lockKey='main';if(syncLocks.has(lockKey))return syncLocks.get(lockKey);const task=(async()=>{const started=Date.now(),db=supabaseAdmin();let config=await getConfig();if(!force&&config?.clan_id&&config?.current_season){const last=await db.from('rep_tracker_sync_runs').select('completed_at,status').eq('clan_id',config.clan_id).order('completed_at',{ascending:false}).limit(1).maybeSingle();const lastCompleted=last.data?.completed_at?Date.parse(last.data.completed_at):0;if(lastCompleted&&Date.now()-lastCompleted<Math.max(30,config.sync_interval_seconds||45)*1000)return{reused:true,config,season:config.current_season||null,membersSeen:Number(last.data?.members_returned||0),lastCompleted:last.data?.completed_at||null};}let discovery;try{discovery=await discoverChaos();config=await updateConfig({clan_id:discovery.clanId,clan_name:discovery.clanName,current_season:discovery.currentSeason||config?.current_season||null,final_day_at:discovery.finalDayAt||config?.final_day_at||null,expected_member_count:discovery.expectedMemberCount||config?.expected_member_count||null});}catch(error){await audit('Sync discovery failed',{message:error instanceof Error?error.message:String(error)},admin);throw new Error(`Clan discovery failed: ${error instanceof Error?error.message:String(error)}`);}const season=config.current_season||discovery.currentSeason;if(!season)throw new Error('Live source did not expose a season and no season is configured.');await ensureSeason(config,season,config.clan_id,discovery.capturedAt);const run=await db.from('rep_tracker_sync_runs').insert({status:'failed',source:'Ninja Zenshin',season,clan_id:config.clan_id,started_at:new Date(started).toISOString()}).select('*').single();const runId=run.data?.id;try{const live=await fetchLiveMembers(config.clan_id),capturedAt=live.fetchedAt||nowIso(),expected=Number(config.expected_member_count||0),returned=live.members.length,memberIds=live.members.map((m)=>String(m.id)).filter(Boolean);if(!memberIds.length)throw new Error('Live source returned members without stable member IDs.');if(expected>0&&returned<expected)throw new Error(`Incomplete live roster: ${returned} returned, ${expected} expected.`);const previous=await previousSnapshotMap(config.clan_id,season,memberIds,capturedAt);const snapshotRows=live.members.map((member)=>{const prev=previous.get(String(member.id)),previousRep=prev?Number(prev.reputation):null,suspicious=previousRep!==null&&Number(member.reputation)<previousRep;return{clan_id:config.clan_id,season,member_id:String(member.id),ign:member.name,level:asInt(member.level),reputation:asInt(member.reputation),stamina:member.stamina==null?null:asInt(member.stamina),max_stamina:member.maxStamina==null?null:asInt(member.maxStamina),source:live.service==='legacy-live'?'Ninja Zenshin public member endpoint':'Ninja Zenshin AMF',captured_at:capturedAt,suspicious,suspicious_reason:suspicious?`REP decreased from ${previousRep} to ${member.reputation}. Raw value retained for audit.`:null,raw_data:member};});const changedSnapshotRows=snapshotRows.filter((row)=>{
...
export async function dashboardData(){
  ...
  const projectedDailyGain=todayGain;
  const targetGap=global?.above?.gap||0;
  const targetEtaHours = targetGap>0 && projectedDailyGain>0 ? (targetGap/(projectedDailyGain/24)) : null;
  return{
