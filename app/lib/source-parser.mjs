...
export function parseCountdown($){const root=$('.clr-cd').first();if(!root.length)return null;const values=['d','h','m','s'].map((key)=>root.find(`[data-${key}]`).first().text().trim());if(values.some((value)=>!value||value==='--'))return null;const parsed=values.map((value)=>toNumber(value));if(parsed.some((value,index)=>!Number.isFinite(value)||String(values[index]).includes('--')))return null;const [days,hours,minutes,seconds]=parsed;return{days,hours,minutes,seconds,remainingSeconds:days*86400+hours*3600+minutes*60+seconds};}
