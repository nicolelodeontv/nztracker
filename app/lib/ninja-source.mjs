...
async function fromLegacy(clanId){
  ...
  try{
    ...
    let members;
    try{const payload=JSON.parse(text),rawMembers=Array.isArray(payload?.members)?payload.members:Array.isArray(payload)?payload:[];members=normalizeMembers(rawMembers);}
    catch{members=normalizeMembers(parseLegacyMemberHtml(text));}
    if(!members.length)throw new Error('Legacy member source returned no valid members.');
    return{clanId,members,count:members.length,fetchedAt:new Date().toISOString(),source:target,service:'legacy-live',stale:false,sourceDiagnostics:{legacy:{status:'success',httpStatus:response.status,durationMs:Date.now()-started}}};
  }catch(error){
    const message=error instanceof Error?error.message:String(error);
    const enriched=error instanceof Error?error:new Error(message);
...
export async function fetchLiveMembers(clanId){
  ...
  const request=(async()=>{
    ...
    try{return await fromAmf(key);}
      ...
      try{
        const legacy=await fromLegacy(key);
        return{...legacy,fallbackReason:amfError instanceof Error?amfError.message:String(amfError),sourceDiagnostics:{amf:amfDiagnostic,legacy:legacy.sourceDiagnostics?.legacy||null},sourceHealth:'degraded'};
      }catch(legacyError){
        const legacyDiagnostic=legacyError?.sourceDiagnostic||{status:'error',durationMs:null,error:legacyError instanceof Error?legacyError.message:String(legacyError)};
        ...
        error.sourceDiagnostics={amf:amfDiagnostic,legacy:legacyDiagnostic};
        throw error;
      }
    }
  })();
