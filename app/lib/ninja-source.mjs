...
async function fetchWithTimeout(url, options = {}) {
  ...
  try {
    ...
    if (error?.name === 'AbortError') throw new Error(`Upstream request timed out after ${UPSTREAM_TIMEOUT_MS / 1000}s.`);
    throw error;
  } finally {
    clearTimeout(timer);
  }
...
export function buildMemberRequest(clanId) {
  const output = [];
...
export function normalizeMembers(rawMembers) {
  ...
  return (Array.isArray(rawMembers) ? rawMembers : []).map((member) => {
    ...
    const reputation = toNumber(source.reputation ?? source.rep ?? source.points) ?? null;
    ...
    const id = clean(source.id ?? source.memberId ?? source.member_id);
    return {
      id,
      name,
    ...
    if (!member.name || member.reputation === null) return false;
...
async function fromAmf(clanId) {
  const response = await fetchWithTimeout(AMF_ORIGIN, {
    method: 'POST', cache: 'no-store', body: buildMemberRequest(clanId),
    headers: {
      ...
      Pragma: 'no-cache', Referer: RANKING_SOURCE,
  ...
  return { clanId, members, count: members.length, fetchedAt: new Date().toISOString(), source: AMF_ORIGIN, service: SERVICE, stale: false };
}
...
function parseLegacyMemberHtml(text) {
  ...
  for (const row of rows) {
    ...
    if (!name) continue;
    parsed.push({ id: '', name, level: toNumber(cells[2]) ?? 0, reputation: toNumber(cells[3]) ?? null });
  }
  return parsed;
}
...
async function fromLegacy(clanId) {
  ...
  const response = await fetchWithTimeout(target, { cache: 'no-store', headers: { Accept: 'text/html,application/json,text/plain,*/*', 'User-Agent': 'Mozilla/5.0 NinjaZenshinLiveTracker/3.0' } });
  ...
  try {
    ...
    members = normalizeMembers(parseLegacyMemberHtml(text));
  }
  if (!members.length) throw new Error('Legacy member source returned no valid members.');
  return { clanId, members, count: members.length, fetchedAt: new Date().toISOString(), source: target, service: 'legacy-live', stale: false };
}
...
export async function fetchLiveMembers(clanId) {
  ...
  const request = (async () => {
    try { return await fromAmf(key); }
      ...
      try { return { ...(await fromLegacy(key)), fallbackReason: amfError instanceof Error ? amfError.message : String(amfError) }; }
      catch (legacyError) {
        const error = new Error(`Live member sources failed. AMF: ${amfError instanceof Error ? amfError.message : String(amfError)} Legacy: ${legacyError instanceof Error ? legacyError.message : String(legacyError)}`);
        error.cause = amfError;
        throw error;
      }
    }
  })();
...
export async function discoverChaos() {
  const response = await fetchWithTimeout(`${RANKING_SOURCE}&_nz=${Date.now()}`, {
    cache: 'no-store',
    headers: { Accept: 'text/html,application/xhtml+xml,text/plain;q=0.9,*/*;q=0.8', 'Cache-Control': 'no-cache', Pragma: 'no-cache', 'User-Agent': 'Mozilla/5.0 NinjaZenshinLiveTracker/3.0' }
  });
  ...
  const finalDayAt = Number.isFinite(countdownSeconds) && countdownSeconds >= 0
    ? new Date(new Date(capturedAt).getTime() + countdownSeconds * 1000).toISOString()
    : null;
  return {
    clanId: String(row.clanId), clanName: row.clan, expectedMemberCount: row.memberCurrent || null,
    currentSeason: parsed.season || null, finalDayAt, countdown: parsed.countdown || null,
    capturedAt, source: RANKING_SOURCE, row,
  };
}
