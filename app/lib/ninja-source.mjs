...
async function fromAmf(clanId){const response=await fetchWithTimeout(AMF_ORIGIN,{method:'POST',cache:'no-store',body:buildMemberRequest(clanId),headers:{Accept:'*/*','Cache-Control':'no-cache','Content-Type':'application/x-amf',Origin:process.env.GAME_SOURCE_ORIGIN||'https://ninjazenshin.online',Pragma:'no-cache',Referer:RANKING_SOURCE,'User-Agent':'Mozilla/5.0 NinjaZenshinLiveTracker/3.0'}}),bytes=new Uint8Array(await response.arrayBuffer());if(!response.ok){const error=new Error(`AMF service returned HTTP ${response.status}.`);error.status=response.status;throw error;}if(!bytes.length)throw new Error('AMF service returned an empty response.');const bodyData=parseMemberResponse(bytes);if(!bodyData||typeof bodyData!=='object')throw new Error('AMF response did not contain an object result.');if(bodyData.status&&String(bodyData.status)!=='1')throw new Error(`Member service returned status ${bodyData.status}.`);const rawMembers=Array.isArray(bodyData.result)?bodyData.result:Array.isArray(bodyData.members)?bodyData.members:[];const members=normalizeMembers(rawMembers);if(!members.length)throw new Error('AMF member result contained no valid members.');return{clanId,members,count:members.length,fetchedAt:new Date().toISOString(),source:AMF_ORIGIN,service:SERVICE,stale:false};}
...
async function fromLegacy(clanId){const target=`${LEGACY_MEMBER_API}${encodeURIComponent(clanId)}?t=${Date.now()}`,response=await fetchWithTimeout(target,{cache:'no-store',headers:{Accept:'text/html,application/json,text/plain,*/*','User-Agent':'Mozilla/5.0 NinjaZenshinLiveTracker/3.0'}});if(!response.ok){const error=new Error(`Legacy member source returned HTTP ${response.status}.`);error.status=response.status;throw error;}const text=await response.text();let members;try{const payload=JSON.parse(text),rawMembers=Array.isArray(payload?.members)?payload.members:Array.isArray(payload)?payload:[];members=normalizeMembers(rawMembers);}catch{members=normalizeMembers(parseLegacyMemberHtml(text));}if(!members.length)throw new Error('Legacy member source returned no valid members.');return{clanId,members,count:members.length,fetchedAt:new Date().toISOString(),source:target,service:'legacy-live',stale:false};}
