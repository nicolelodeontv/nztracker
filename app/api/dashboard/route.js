...
export async function GET() {
  try {
    ...
    const activity = await recentActivity(10, data);
    return Response.json({ ok: true, ...data, activity, syncError: null, serverTime: new Date().toISOString() }, { headers: { 'Cache-Control': 'no-store, max-age=0' } });
  } catch (error) {
