  ...
  return <section className="war-next-build">
    <div className="next-grid">
      <section className="next-panel recommendation">
        ...
        <div className="attack-stamina-card"><div><small>YOUR ATTACK STAMINA</small><strong>{attackStamina} / {DEFAULT_MAX_STAMINA}</strong><span>{attackPercent}% · {ATTACK_STAMINA_COST} stamina per attack</span></div><div className="attack-stamina-meter"><i style={{ width: `${attackPercent}%` }} /></div><div className="attack-stamina-actions"><button onClick={resetAttackStamina}>Reset to 200</button><button onClick={() => setAttackStamina((value) => clampStamina(value - ATTACK_STAMINA_COST))} disabled={attackStamina < ATTACK_STAMINA_COST}>−{ATTACK_STAMINA_COST} Attack</button></div></div>

        {bestRow ? <div className="recommended-card"><div className="recommended-title"><span>#1</span><b>{bestRow.clan}</b><strong>{bestRow.reward == null ? 'ENTER REP' : `+${bestRow.reward} REP`}</strong></div><div className="recommended-meta"><span>🔴 BLEEDING</span><span>{confidence(bestRow).pct}% stamina verified</span><span>{bestRow.status.bleedingMembers || 0}/{bestRow.status.memberCount || 0} below threshold</span></div><div className="recommended-actions"><button onClick={()=>{setSelected(bestRow.clan);recordAttack(true);}}>Use Recommended Target</button></div></div> : <div className="next-empty">No confirmed Bleeding target available. The assistant will not recommend a target with partial or unknown stamina.</div>}
      </section>

      <section className="next-panel confidence-panel"><div className="next-head"><div><small>SOURCE CONFIDENCE</small><h2>Stamina Verification</h2></div><span>real source coverage</span></div><div className="confidence-list">{entries.slice(0, 8).map((row) => { const c = confidence(row); return <div className="confidence-row" key={row.clan}><span><b>{row.clan}</b><small>{row.status.state === 'bleeding' ? '🔴 BLEEDING' : row.status.state === 'potential-bleeding' ? '🟡 POTENTIAL' : row.status.state === 'healthy' ? '🟢 HEALTHY' : '⚪ UNKNOWN'}</small></span><strong>{c.pct}%</strong></div>; })}</div></section>
    </div>
