...
export default function ClanDetailPage({ params }) {
  ...
  return <main className="tracker">
    ...
    <section className="stats"><div className="card"><div className="eyebrow">REPUTATION</div><strong>{fmt(clan.reputation)}</strong><small>Current ranking</small></div><div className="card"><div className="eyebrow">STAMINA VERIFIED</div><strong>{verified}/{members.length || clan.memberCurrent || 0}</strong><small>{members.length ? `${Math.round(verified / members.length * 100)}% source coverage` : 'No member data'}</small></div><div className="card"><div className="eyebrow">LOW STAMINA</div><strong>{low}</strong><small>At/below personal 70% threshold</small></div><div className="card"><div className="eyebrow">STATUS</div><strong>{members.length && verified === members.length && low >= Math.ceil(members.length * .5) ? '🔴 BLEEDING' : verified ? low ? '🟡 POTENTIAL' : '🟢 HEALTHY' : '⚪ UNKNOWN'}</strong><small>Last sync {updated?.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'})}</small></div></section>
    <section className="section"><div className="section-head"><div><div className="eyebrow">LIVE MEMBERS</div><h2>Stamina & Reputation</h2><p>Individual thresholds use 70% of each member's Max Stamina.</p></div></div><div className="table-wrap"><div className="table-head" style={{minWidth:'760px',gridTemplateColumns:'minmax(180px,1fr) 70px 120px 120px 100px'}}><span>MEMBER</span><span>LEVEL</span><span>REPUTATION</span><span>STAMINA</span><span>STATUS</span></div>{members.map((m, i)=><div className="table-row" key={`${m.name}-${i}`} style={{minWidth:'760px',gridTemplateColumns:'minmax(180px,1fr) 70px 120px 120px 100px'}}><span className="clan-cell"><b>{m.name}</b></span><span>{m.level || '—'}</span><span>{fmt(m.reputation)}</span><span>{m.stamina != null && m.maxStamina != null ? `${fmt(m.stamina)} / ${fmt(m.maxStamina)}` : 'UNKNOWN'}</span><span className={m.bleeding ? 'gain' : 'muted'}>{m.bleeding == null ? 'UNKNOWN' : m.bleeding ? '🔴 LOW' : '🟢 OK'}</span></div>)}</div></section>
  </main>;
}
