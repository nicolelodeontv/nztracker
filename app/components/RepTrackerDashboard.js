...
export default function RepTrackerDashboard({ initialView = 'dashboard' }) {
  ...
  return <main className="ops-app">
    ...
    {view==='members'&&    {view==='admin'&&<section><div className="page-head"><div><span className="eyebrow">ADMINISTRATION</span><h1>CONTROL ROOM</h1><p>Protected actions. Historical snapshots are never silently replaced.</p></div></div>{adminContent}</section>}K</th><th>MEMBER</th><th>LV</th><th>FINAL REP</th><th>GAIN</th><th>HOURS</th><th>REP / HR</th></tr></thead><tbody>{(latestFinal.raw_snapshot?.rows||[]).map((r,i)=><tr key={r.id||i}><td>#{i+1}</td><td>{r.member}</td><td>{r.level}</td><td>{fmt(r.rep)}</td><td>+{fmt(r.gain)}</td><td>{fmtHours(r.hours)}</td><td>{fmt(r.repPerHour)}</td></tr>)}</tbody></table></div></div></>:<div className="empty-state compact"><h3>FINAL DAY NOT LOCKED</h3><p>Locking requires a fresh successful upstream sync and a complete expected roster.</p></div>}</section>}
