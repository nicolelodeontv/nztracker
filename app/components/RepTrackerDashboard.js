    ...
    <aside ref={drawerRef} className="drawer" role="dialog" aria-modal="true" aria-labelledby="member-drawer-title" tabIndex="-1" onMouseDown={(event)=>event.stopPropagation()}>
      ...
      {!data?<div className="loading">LOADING LIVE HISTORY…</div>:<>
        <div className="mini-stats">
          ...
          {summary.stamina!=null&&<div><span>STAMINA</span><b>{fmt(summary.stamina)+'/'+fmt(summary.maxStamina||200)}</b></div>}
        </div>
        {summary.stamina!=null
          ? <div className="drawer-stamina-note stamina-disclaimer">SERVER-REPORTED STAMINA FROM THE GAME SOURCE.</div>
          : <div className="drawer-stamina-note stamina-hold-note">STAMINA DATA UNAVAILABLE · NO REP-DERIVED ESTIMATE IS SHOWN. DO NOT USE STAMINA DATA FOR REWARD OR BLEEDING DECISIONS.</div>}
...
export default function RepTrackerDashboard({ initialView = 'dashboard', initialData = null, initialError = '' }) {
  ...
  const showStaminaColumn = Boolean(data?.staminaSummary?.trackingReady && data?.staminaSummary?.mode==='SERVER_REPORTED');
  ...
  if (dashboardLoading && !data) return <main className="ops-app"><header className="ops-header"><Link className="brand" href="/" aria-label="Go to Dashboard"><img className="brand-logo" src="/chaos-helmet-logo.svg" alt="CHAOS samurai helmet"/><div><b>CHAOS</b>{' '}<span>REP TRACKER</span><small>Ninja Zenshin Clan Operations</small></div></Link></header><section className="dashboard-skeleton" aria-label="Loading dashboard"><div className="skeleton-bar wide"></div><div className="skeleton-stats">{Array.from({length:6}).map((_,i)=><div className="skeleton-block" key={i}><span></span><b></b></div>)}</div><div className="panel skeleton-table"><div className="skeleton-bar"></div>{Array.from({length:8}).map((_,i)=><div className="skeleton-row" key={i}><span></span><span></span><span></span><span></span></div>)}</div></section></main>;

  if (dashboardError && !data) return <main className="ops-app"><header className="ops-header"><Link className="brand" href="/" aria-label="Go to Dashboard"><img className="brand-logo" src="/chaos-helmet-logo.svg" alt="CHAOS samurai helmet"/><div><b>CHAOS</b>{' '}<span>REP TRACKER</span><small>Ninja Zenshin Clan Operations</small></div></Link></header><section className="empty-state"><span className="eyebrow">DASHBOARD ERROR</span><h1>Unable to load live dashboard data.</h1><p>{dashboardError}</p><button className="btn primary" onClick={() => refresh({ initial: true })}>RETRY</button></section></main>;

  if (data && data.configured === false) return <main className="ops-app"><header className="ops-header"><Link className="brand" href="/" aria-label="Go to Dashboard"><img className="brand-logo" src="/chaos-helmet-logo.svg" alt="CHAOS samurai helmet"/><div><b>CHAOS</b>{' '}<span>REP TRACKER</span><small>Ninja Zenshin Clan Operations</small></div></Link><button className="btn primary" onClick={() => setLoginOpen(true)}>ADMIN SETUP</button></header><section className="empty-state"><span className="eyebrow">NO LIVE CONFIGURATION</span><h1>Waiting for a real Ninja Zenshin clan source.</h1><p>The tracker will not fabricate roster, REP, season, or countdown values. Open Admin to authenticate and run discovery.</p>{message&&<div className="notice bad">{message}</div>}</section>{loginOpen&&<ThemedModal variant="prompt" eyebrow="SECURE ADMIN" title="ADMIN ACCESS" message="Admin actions modify persistent tracking state and finalization status." inputType="password" inputValue={password} onInputChange={setPassword} placeholder="Admin password" confirmLabel="SIGN IN" onConfirm={login} onCancel={()=>{setLoginOpen(false);setPassword('');}} />}</main>;
  ...
  return <main className="ops-app">
    <header className="ops-header">
      <Link className="brand" href="/" aria-label="Go to Dashboard"><img className="brand-logo" src="/chaos-helmet-logo.svg" alt="CHAOS samurai helmet"/><div><b>CHAOS</b>{' '}<span>REP TRACKER</span><small>Ninja Zenshin Clan Operations</small></div></Link>
