    ...
    <aside ref={drawerRef} className="drawer" role="dialog" aria-modal="true" aria-labelledby="member-drawer-title" tabIndex="-1" onMouseDown={(event)=>event.stopPropagation()}>
      ...
      {!data?<div className="loading">LOADING LIVE HISTORY…</div>:<>
        <div className="mini-stats">
          ...
          <div><span>STAMINA</span><b>{summary.stamina==null?'—':fmt(summary.stamina)+'/'+fmt(summary.maxStamina||200)}</b></div>
        </div>
        <div className="drawer-stamina-note">
          {summary.stamina==null
            ? 'CALCULATED STAMINA NOT YET INITIALIZED'
            : 'CALCULATED FROM REP ACTIVITY · NOT SERVER-REPORTED STAMINA'}
        </div>
