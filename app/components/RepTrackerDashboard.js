...
export default function RepTrackerDashboard({ initialView = 'dashboard', initialData = null, initialError = '' }) {
  ...
  function requestLockFinal(){
    setThemedModal({
      ...
      title:'FINALIZE SEASON RESULTS?',
      message:'This creates an immutable final snapshot. The finalized results can only be corrected by creating a new version.',
      confirmLabel:'LOCK FINAL DAY',
      cancelLabel:'CANCEL',
      destructive:true,
      onConfirm:async()=>{setThemedModal(null);await lockFinal();}
    });
  }
