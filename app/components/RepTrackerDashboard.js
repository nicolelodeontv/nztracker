...
function MemberDrawer({ member, donation, onClose }) {
  ...
  return <div className="drawer-backdrop" onClick={onClose}><aside className="drawer" onClick={e => e.stopPropagation()}>
    ...
    {!data ? <div className="loading">LOADING HISTORY…</div> : <>
      ...
      <div className="panel inset"><div className="section-title"><div><span className="eyebrow">DONATIONS</span><h3>GOLD / TOKEN</h3></div><span>{donation ? 'LATEST SNAPSHOT' : 'NO SNAPSHOT'}</span></div><div className="mini-stats donation"><div><span>DONATED GOLD</span><b>{donation ? fmt(donation.donated_gold) : '—'}</b></div><div><span>DONATED TOKEN</span><b>{donation ? fmt(donation.donated_token) : '—'}</b></div><div><span>1H GAIN</span><b>{donationPair(donation?.gains?.oneHour)}</b></div><div><span>6H GAIN</span><b>{donationPair(donation?.gains?.sixHour)}</b></div><div><span>24H GAIN</span><b>{donationPair(donation?.gains?.twentyFourHour)}</b></div><div><span>PREVIOUS</span><b>{donationPair(donation?.gains?.previous)}</b></div></div></div>
...
export default function RepTrackerDashboard({ initialView = 'dashboard' }) {
  ...
  return <main className="ops-app">
    ...
    {view==='dashboard'&&<>
      ...
      <section className="panel table-panel"><div className="section-title"><div><span className="eyebrow">LIVE MEMBER RANKING</span><h2>REP PERFORMANCE</h2></div><span>{rows.length} members · source {rows[0]?.source || '—'}</span></div><div className="table-scroll"><table><thead><tr><th>RANK</th><th>MEMBER</th><th>LV</th><th>CURRENT REP</th><th>REP GAIN</th><th>DONATED GOLD</th><th>DONATED TOKEN</th><th>PREV G/T</th><th>1H G/T</th><th>6H G/T</th><th>24H G/T</th><th>HOURS</th><th>REP / HR</th><th>STATUS</th></tr></thead><tbody>{rows.map((r,i)=><tr key={r.id} onClick={()=>setSelected(r)}><td>#{i+1}</td><td className="member-name">{r.member}</td><td>{r.level}</td><td className="num">{fmt(r.rep)}</td><td className={r.gain>0?'up':''}>{r.gain>0?`+${fmt(r.gain)}`:fmt(r.gain)}</td><td>{r.donation ? fmt(r.donation.donated_gold) : '—'}</td><td>{r.donation ? fmt(r.donation.donated_token) : '—'}</td><td>{donationPair(r.donation?.gains?.previous)}</td><td>{donationPair(r.donation?.gains?.oneHour)}</td><td>{donationPair(r.donation?.gains?.sixHour)}</td><td>{donationPair(r.donation?.gains?.twentyFourHour)}</td><td>{fmtHours(r.hours)}</td><td>{fmt(r.repPerHour)}</td><td><span className={`status ${r.status}`}>{r.suspicious?'SUSPICIOUS':r.status.toUpperCase()}</span></td></tr>)}</tbody></table>{!rows.length&&<div className="chart-empty">NO LIVE DATA</div>}</div></section>
