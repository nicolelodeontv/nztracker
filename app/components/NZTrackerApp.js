...
function rankingAgeLabel(updatedAt, now) {
...
export default function NZTrackerApp() {
  ...
  const refreshOps = useCallback(async () => {
    try {
      const bust = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
      const [syncData, healthData] = await Promise.all([readJson(`/api/sync-status?refresh=${bust}`), readJson(`/api/health?refresh=${bust}`)]);
  ...
  const refreshSelected = useCallback(async (clan) => {
    ...
    try {
      ...
      setMembers(Array.isArray(data.members) ? data.members : []); setMemberState(data.stale ? 'STALE' : 'LIVE');
  ...
  useEffect(() => { if (rankingState === 'LIVE') { if (retryRef.current) clearInterval(retryRef.current); retryRef.current = null; return undefined; } if (!retryRef.current) retryRef.current = setInterval(refreshRanking, RANKING_RETRY_MS); return () => { if (retryRef.current) clearInterval(retryRef.current); retryRef.current = null; }; }, [rankingState, refreshRanking]);
  ...
  const intel = useMemo(() => { const gain = rows.reduce((sum, row) => sum + row.gain, 0); const active = rows.filter((row) => row.status === 'ACTIVE').length; const recent = rows.filter((row) => row.status === 'RECENT').length; const idle = rows.filter((row) => row.status === 'IDLE').length; const noGain = rows.filter((row) => row.status === 'NO GAIN').length; const missing = rows.filter((row) => row.status === 'MISSING').length; const reset = rows.filter((row) => row.status === 'RESET').length; const top = [...rows].sort((a, b) => b.gain - a.gain).slice(0, 5); const burn = rows.filter((row) => row.current < 10000).sort((a, b) => a.current - b.current); return { gain, hour: gain / Math.max(periodHours, 1), active, recent, idle, noGain, missing, reset, top, burn }; }, [rows, periodHours]);
  const countdown = useMemo(() => { const total = Math.max(0, Math.floor((new Date(seasonEnd).getTime() - syncedNow) / 1000)); return `${Math.floor(total / 86400)}d ${String(Math.floor(total / 3600) % 24).padStart(2, '0')}h ${String(Math.floor(total / 60) % 60).padStart(2, '0')}m ${String(total % 60).padStart(2, '0')}s`; }, [seasonEnd, syncedNow]);
