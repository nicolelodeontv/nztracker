...
function updateRepState(rows){
  ...
  newChanges.reverse().forEach(change=>{const prev=changeFeed[0];if(!prev||prev.clan!==change.clan||prev.delta!==change.delta||change.at-prev.at>1500)changeFeed.unshift(change);});changeFeed.splice(18);write(REP_KEY,state);return state;
}
...
function renderCommandCenter(rows,state){
  ...
  const recent=changeFeed.slice(0,3).map(change=>`<span class="command-change">${change.clan} <strong>${change.delta>=0?'+':'−'}${Math.abs(change.delta).toLocaleString('en-US')}</strong></span>`).join('');bar.querySelector('[data-recent-changes]').innerHTML=recent?`Recent: ${recent}`:'Recent: —';applyFilter(bar.querySelector('.command-tab.active')?.dataset.view||'all');
}
...
function renderFeed(rows){
  const feed=document.querySelector('[data-nz-feed]');if(!feed)return;const list=feed.querySelector('.nz-feed-list');const top=feed.querySelector('.nz-top-list');list.innerHTML=changeFeed.length?changeFeed.slice(0,18).map(item=>`<div class="nz-feed-item"><span class="nz-feed-time">${new Date(item.at).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'})}</span><span class="nz-feed-clan">${item.clan}</span><span class="nz-feed-value ${item.delta>=0?'up':'down'}">${item.delta>=0?'+':'−'}${Math.abs(item.delta).toLocaleString('en-US')} REP</span></div>`).join(''):'<div class="empty">Waiting for the first reputation change…</div>';const topRows=[...rows].sort((a,b)=>num(b.children?.[5]?.textContent)-num(a.children?.[5]?.textContent)).slice(0,5);top.innerHTML=topRows.length?topRows.map((row,index)=>`<div class="nz-top-item"><span class="nz-top-rank">#${index+1}</span><span class="nz-top-name">${rowClan(row)}</span><span class="nz-top-gain">+${num(row.children?.[5]?.textContent).toLocaleString('en-US')}</span></div>`).join(''):'<div class="empty">Waiting for gain data…</div>';
}
...
function buildMobileCards(rows){
  const table=document.querySelector('.table-wrap');if(!table)return;let wrap=document.querySelector('.nz-mobile-card');
  if(!wrap || wrap.dataset.rowCount !== String(rows.length)){
    wrap?.remove();wrap=document.createElement('div');wrap.className='nz-mobile-card';wrap.dataset.rowCount=String(rows.length);rows.forEach(row=>{const item=document.createElement('div');item.className='nz-mobile-item';item.dataset.clan=rowClan(row);item.innerHTML='<div class="nz-mobile-top"><div class="nz-mobile-clan"><b></b><small></small></div><span class="nz-mobile-rank"></span></div><div class="nz-mobile-grid"><div class="nz-mobile-stat"><small>MEMBERS</small><b></b></div><div class="nz-mobile-stat"><small>REPUTATION</small><b></b></div><div class="nz-mobile-stat"><small>GAIN</small><b class="gain"></b></div></div>';wrap.appendChild(item);item.addEventListener('click',()=>{const current=[...document.querySelectorAll('.table-wrap .table-row')].find(candidate=>rowClan(candidate)===item.dataset.clan);current?.click();});});table.after(wrap);
  }
  refreshMobileCards(document.querySelector('.command-tab.active')?.dataset.view||'all');
}
...
export default function SiteEnhancements(){useEffect(()=>{let frame=0;const schedule=()=>{cancelAnimationFrame(frame);frame=requestAnimationFrame(enhance)};const observer=new MutationObserver(schedule);observer.observe(document.body,{childList:true,subtree:true});const timer=setInterval(schedule,1200);schedule();return()=>{observer.disconnect();clearInterval(timer);cancelAnimationFrame(frame);};},[]);return null;}
