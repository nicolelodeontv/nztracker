...
const parseClanId = (html) => {
  const dataMatch = html.match(/(?:data-clan-id|data-clan|clan_id|clanid)=[\"']([^\"']+)[\"']/i);
  if (dataMatch?.[1]) return dataMatch[1];

  const attrMatch = html.match(/(?:data-id|data-clan-id)=[\"']([^\"']+)[\"']/i);
  if (attrMatch?.[1]) return attrMatch[1];

  const memberLinkMatch = html.match(/clan-ranking\/members\/([^\"'?#\s]+)/i);
  return memberLinkMatch?.[1] || null;
};
...
export function parseRankingHtml(html) {
  ...
  for (const match of tableMatches) {
    ...
    const idMatch = rowHtml.match(/(?:data-clan-id|data-clan|clan_id|clanid)=[\"']([^\"']+)[\"']/i)
      || rowHtml.match(/clan-ranking\/members\/([^\"'?#\s]+)/i);
