...
export async function upsertMemberRoster({ season, snapshotAt, clanResults }) {
  ...
  for (const result of successful) {
    for (let index = 0; index < result.members.length; index += 1) {
      ...
      const record = {
        ...
        source: result.source || GAME_SOURCE,
        captured_at: result.fetchedAt || snapshotAt,
        snapshot_at: snapshotAt,
        raw_data: member.rawData || member
      };
      records.push(record);
      if (!previous) {
        joined += 1;
        events.push({ clan_id: String(result.clanId), season, member_id: memberId, member_name: name, event_type: 'joined', occurred_at: snapshotAt, previous_data: null, current_data: record });
      }
    }
  }
  ...
  for (const member of prior || []) {
    ...
    list.push(member);
    previousByClan.set(String(member.clan_id), list);
  }
  for (const result of successful) {
    ...
    for (const previous of previousByClan.get(String(result.clanId)) || []) {
      if (!currentClanKeys.has(String(previous.member_id))) {
        left += 1;
        events.push({ clan_id: String(result.clanId), season, member_id: String(previous.member_id), member_name: String(previous.name), event_type: 'left', occurred_at: snapshotAt, previous_data: previous, current_data: null });
      }
    }
  }
...
export async function getLeaderboards({ type, season, round, limit = 100 }) {
  const db = dbOrNull();
  if (!db) return null;
  let query = db.from('leaderboard_entries')
    .select('board_type,season,round,rank,player_name,score,wins,losses,title,badge,source,captured_at')
    .eq('board_type', type)
  ...
  if (error) throw error;
  return (data || []).map((row) => ({ boardType: row.board_type, season: row.season, round: row.round, rank: row.rank, playerName: row.player_name, score: row.score, wins: row.wins, losses: row.losses, title: row.title, badge: row.badge, source: row.source, capturedAt: row.captured_at }));
}

export async function getLeaderboardHistory({ type, playerName, season, limit = 200 }) {
  const db = dbOrNull();
  if (!db) return null;
  let query = db.from('leaderboard_history')
    .select('board_type,season,round,rank,player_name,score,wins,losses,title,badge,snapshot_at')
    .eq('board_type', type)
...
export async function getClanHistory({ clanId, season, hours = 168, limit = 500 }) {
  ...
  const { data, error } = await db.from('clan_ranking_history')
    .select('clan_id,season,rank,clan_name,master,member_current,member_max,reputation,snapshot_at')
    .eq('clan_id', String(clanId))
    .eq('season', season || 'Season 3')
...
export async function getCurrentMembers({ clanId, season, limit = 1000 }) {
  const db = dbOrNull();
  if (!db) return null;
  const { data, error } = await db.from('clan_members')
    .select('clan_id,season,member_id,name,level,reputation,stamina,max_stamina,title,badge,source,captured_at,snapshot_at')
    .eq('clan_id', String(clanId))
    .eq('season', season || 'Season 3')
