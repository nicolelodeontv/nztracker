...
import { latestByKey, selectHistoryWrites } from '../app/lib/history-sparsity.mjs';
...
async function getRecentLeaderboardHistory({ type, season, round, playerNames, snapshotAt }) {
  const db = dbOrNull();
  if (!db || !season || !playerNames.length) return new Map();
  const since = new Date((Date.parse(snapshotAt) || Date.now()) - 2 * 60 * 60 * 1000).toISOString();
  const { data, error } = await db.from('leaderboard_history')
    .select('board_type,season,round,player_name,rank,score,wins,losses,snapshot_at')
    .eq('board_type', type)
    .eq('season', season)
    ...
    .limit(Math.min(10000, Math.max(1000, playerNames.length * 50)));
  if (error) throw error;
  return latestByKey(
    data || [],
    (row) => `${row.board_type}:${row.season}:${row.round}:${row.player_name}`
  );
}
...
export async function upsertLeaderboardRows({ type, season, round = '', rows, capturedAt }) {
  ...
  const playerNames = [...new Set(records.map((record) => record.player_name))];
  const previous = await getRecentLeaderboardHistory({
    type,
    season,
    round: normalizedRound,
    playerNames,
  ...
  const selected = selectHistoryWrites(records, previous, {
    keyOf: (record) => `${record.board_type}:${record.season}:${record.round}:${record.player_name}`,
    snapshotAt: capturedAt,
    trackedFields: ['rank', 'score', 'wins', 'losses']
  });
  ...
  const historyRecords = selected.rows.map(({ captured_at, updated_at, ...record }) => ({
    ...record,
...
async function getRecentClanHistory({ season, records, snapshotAt }) {
  const db = dbOrNull();
  if (!db || !season || !records.length) return new Map();
  const clanIds = [...new Set(records.map((record) => String(record.clan_id)).filter(Boolean))];
  const since = new Date((Date.parse(snapshotAt) || Date.now()) - 2 * 60 * 60 * 1000).toISOString();
  const { data, error } = await db.from('clan_ranking_history')
    .select('clan_id,season,rank,reputation,member_current,snapshot_at')
    .eq('season', season)
    .in('clan_id', clanIds)
    .gte('snapshot_at', since)
    .order('snapshot_at', { ascending: false })
    .limit(Math.min(10000, Math.max(1000, records.length * 50)));
  if (error) throw error;
  return latestByKey(data || [], (row) => String(row.clan_id));
}
...
export async function recordClanHistory({ rows, season, capturedAt }) {
  ...
  const previous = await getRecentClanHistory({ season, records, snapshotAt: capturedAt });
  const selected = selectHistoryWrites(records, previous, {
    keyOf: (record) => String(record.clan_id),
    snapshotAt: capturedAt,
    trackedFields: ['rank', 'reputation', 'member_current']
  });
  if (!selected.rows.length) {
    return {
      stored: false,
  ...
  const { error } = await db.from('clan_ranking_history').insert(selected.rows);
  if (error) throw error;
  return {
    stored: true,
