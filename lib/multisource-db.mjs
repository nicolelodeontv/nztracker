...
export async function upsertLeaderboardRows({ type, season, round = '', rows, capturedAt }) {
  ...
  const cleanRows = safeRows(rows).filter((row) => Number(row?.rank) > 0);
  ...
  const { error: historyError } = await db.from('leaderboard_history').insert(records.map((record) => ({
    ...record,
    snapshot_at: capturedAt
  })));
  if (historyError) throw historyError;

  return { stored: true, count: records.length, type, season, round };
}
...
export async function upsertMemberRoster({ season, snapshotAt, clanResults }) {
  ...
  for (const result of successful) {
    for (let index = 0; index < result.members.length; index += 1) {
      ...
      if (!previous) {
        joined += 1;
        events.push({
          clan_id: String(result.clanId), season, member_id: memberId, member_name: name,
  ...
  if (records.length) {
    const { error } = await db.from('clan_members').upsert(records, {
  ...
  for (const result of successful) {
    ...
    for (const previous of previousByClan.get(String(result.clanId)) || []) {
      if (!currentClanKeys.has(String(previous.member_id))) {
        left += 1;
        events.push({
          clan_id: String(result.clanId), season, member_id: String(previous.member_id), member_name: String(previous.name),
...
export async function getCurrentMembers({ clanId, season, limit = 1000 }) {
  const db = dbOrNull();
  if (!db) return null;
  let query = db.from('clan_members')
