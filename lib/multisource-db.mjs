  ...
  for (const result of safeRows(clanResults)) {
    ...
    for (const member of safeRows(result.members)) {
      if (member?.identityAmbiguous) {
        ambiguousNames.push({ clanId, name: String(member.name || '').trim() });
        continue;
      }
      ...
      rows.push({
        ...
        source: result.service === 'legacy-live' ? 'Ninja Zenshin public member endpoint' : (result.source || GAME_SOURCE),
...
export async function upsertRepTrackerSnapshots({ season, snapshotAt, clanResults }) {
  ...
  const prepared = buildRepTrackerSnapshotRows({ season, snapshotAt, clanResults });
  if (!prepared.rows.length) {
    return {
      stored: false,
      count: 0,
      ambiguousNames: prepared.ambiguousNames,
      reason: 'No unambiguous member rows to record'
    };
  }
  const { error } = await db.from('rep_tracker_snapshots').upsert(prepared.rows, {
