...
import { createRefreshGate, DASHBOARD_REFRESH_INTERVAL_MS, LIVE_REFRESH_INTERVAL_MS, PERIOD_HISTORY_REFRESH_INTERVAL_MS } from '../app/lib/dashboard-client.mjs';

test('refresh gate uses the full-dashboard interval', async () => {
  assert.equal(DASHBOARD_REFRESH_INTERVAL_MS, 60000);
  assert.equal(LIVE_REFRESH_INTERVAL_MS, 1000);
...
test('dashboard lifecycle regression guard uses declared refresh state and one mount effect', async () => {
  ...
  assert.match(mountEffect[0], /setInterval\(\(\) => refresh\(\), DASHBOARD_REFRESH_INTERVAL_MS\)/);
